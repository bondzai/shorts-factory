...
from factory.generators import base
from factory.series import longform, season as season_mod, trace
...
class FakeGen:
    name, variants, ready, blurb = "fakegen", ["race"], True, "test"

    def generate(self, **kw):
        raise NotImplementedError

    def redraw(self, trace_dir):
        arrays, meta = trace.read(trace_dir)
        shade = int(meta["seed"]) % 255
        for _ in range(arrays["round0_positions"].shape[0]):
            yield bytes([shade]) * (SIM_W * SIM_H * 3)


@pytest.fixture
def season(sandbox, monkeypatch):
    monkeypatch.setitem(base._REGISTRY, "fakegen", FakeGen())
    ...
    fake.table = lambda conn, ch, s, *, before_level=None: table.get(before_level, table["L02"])
...
def test_a_tournament_has_a_card_a_race_and_a_table_per_level(season):
    with db.connect() as conn:
        out = longform.render(conn, "main", season, ["L01", "L02"], "tournament", preset="ultrafast")
