"""Local web UI for the daily loop.
...
class _Job:
    ...
    Deliberately not a queue: two builds at once would fight over the same
    clip rows, and there is never a reason to start one while another runs.
    """
    ...
    def start(self, name: str, work: Callable[[Callable[[str], None]], None]) -> None:
        with self.lock:
            if self.running:
                raise HTTPException(409, f"{self.name} is already running")
            self.name = name
            self.log = []
...
def state() -> dict[str, Any]:
    with db.connect() as conn:
        ...
        queue = [_clip_json(r) for r in db.by_status(conn, AWAITING_APPROVAL)]
        approved = [_clip_json(r) for r in db.by_status(conn, APPROVED)]
        recent = [_clip_json(r) for r in db.recent(conn, 12)]
        total = conn.execute("SELECT SUM(cost_usd) s FROM clips").fetchone()["s"] or 0.0
    return {
        "counts": counts,
        "queue": queue,
        "approved": approved,
...
def plan(body: PlanBody) -> dict[str, Any]:
    def work(emit):
        emit(f"asking the Idea agent for {body.count} clip(s)")
        with db.connect() as conn:
            ids, cost = pipeline.plan(conn, body.count)
            for clip_id in ids:
                row = db.get(conn, clip_id)
...
def build() -> dict[str, Any]:
    def work(emit):
        with db.connect() as conn:
            planned = db.by_status(conn, "planned")
            if not planned:
                emit("nothing planned")
                return
            emit(f"building {len(planned)} clip(s)")
            spend = 0.0
            for row in planned:
...
def digest() -> dict[str, Any]:
    def work(emit):
        with db.connect() as conn:
            body, cost, applied = pipeline.digest(conn, apply_rules=False)
        for line in body.splitlines():
            emit(line)
        emit(f"${cost:.4f}")

    JOB.start("digest", work)
    return JOB.state()
