...
def lead_changes(race) -> list[tuple[int, int]]:
...
def trap_catches(states, radii, style, sim_w: float) -> list[tuple[int, int]]:
    ...
    for i, radius in enumerate(radii):
        inside = [any(stage_qa._in_pit(s[i], t, big) for t in style.traps) for s in states]
        f = 0
        while f < len(states):
            if not inside[f]:
                f += 1
            ...
            while end < len(states) and inside[end]:
                end += 1
...
def placements(round_: dict, running: list[str]) -> list[Placement]:
    ...
    reached. Ties share a rank (1, 1, 3)."""
...
def build(rounds: list[dict], fps: int, sim_w: float, running: list[str]) -> Outcome:
    ...
    for n, r in enumerate(rounds):
        ...
        changes = lead_changes(race_of(r))
        ...
        events += [Event(t_s=at(f), kind="trap_catch", entrant_id=names[i], data={"round": n})
                   for f, i in trap_catches(r["states"], [b.radius for b in r["balls"]], r["style"], sim_w)]
        events += [Event(t_s=at(f), kind="launched", entrant_id=names[i], data={"round": n})
                   for f, i in launches(r["states"], r["style"], fps)]
        offset += r["duration_s"]
    order = {"round_start": 0, "lead_change": 1, "trap_catch": 2, "launched": 3, "finish": 4}
